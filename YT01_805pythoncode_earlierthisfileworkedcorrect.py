...
def generate_dataframe_YT01805(lines_conditions_content,lines_conditions_filename, tpvalue_conditions_content,tpvalue_conditions_filename,df_file_content,df_file_filename,combined_df_content,combined_df_filename):
    ...
    if 'csv' in combined_df_filename:
        ...
        combined_df = pd.read_excel(io.BytesIO(combined_df_content), engine='openpyxl', dtype='object', sheet_name="Combined_TP")
    ...
    def apply_conditions_combined(row, index, conditions, previous_results, use_combined_df=False):
        ...
        if use_combined_df and 'Zone' in row and row['Zone'] in combined_df['Location Code'].values:
            combined_values = combined_df.loc[combined_df['Location Code'] == row['Zone']]
        else:
    ...
    for index, row in df.iterrows():
        df.at[index, 'Result'] = apply_conditions_combined(row, index, Lines_conditions, previous_results, use_combined_df=False)
        previous_results[index] = df.at[index, 'Result']
